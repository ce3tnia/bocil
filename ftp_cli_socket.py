...
TCP_IP = "192.168.43.92" # Only a local server
...
def upld(file_name):
    ...
    # Wait for server ok
        ...
        fsize = struct.pack("h", sys.getsizeof(file_name));
        s.send(fsize)
        print(file_name)
        s.send(file_name.encode())
        # Wait for server ok then send file size
        s.recv(BUFFER_SIZE)
        pathsize = struct.pack("i", os.path.getsize(file_name))
        s.send(pathsize)
    except:
...
def delf(file_name):
    ...
    try:
        ...
        s.send(struct.pack("h", sys.getsizeof(file_name)).encode())
        s.send(file_name.encode())
    except:
